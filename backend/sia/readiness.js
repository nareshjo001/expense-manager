...
const IMPLEMENTED_PROVIDERS = Object.freeze(["openai", "gemini"]);
...
// GEMINI_API_KEY). A provider present in IMPLEMENTED_PROVIDERS but absent
// from this map would be readiness-checked with no credential requirement
// at all, so every entry in IMPLEMENTED_PROVIDERS above must have a
// matching entry here.
 ...
 *        read for presence only, never exposed)
 */
function isSiaReady() {
  ...
  // 3. Model -- llmService.js's askOpenAi()/askGemini() throw MODEL_NOT_CONFIGURED for a blank model, and config.js supplies no default.
