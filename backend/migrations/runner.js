...
function loadMigrations(dir = MIGRATIONS_DIR) {
  return loadMigrationFilenames(dir).map((filename) => {
    ...
    // Always re-read from disk rather than trusting Node's require
    // cache. A failed (not-yet-applied) migration is legitimately
    // edited in place before being retried -- ADR-0006's forward-fix
    // policy only forbids editing an *already-applied* migration, not
    // one that threw and never took effect. Without this, a
    // long-lived process that calls loadMigrations() more than once
    // (a retry loop, an admin tool, this driver's own tests) would
    // silently keep re-running the old, broken code from cache
    // instead of the fix, with nothing to signal it.
