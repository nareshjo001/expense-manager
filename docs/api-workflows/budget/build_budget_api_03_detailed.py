"""
BUDGET-03 Level 2 — detailed implementation workflow for PUT /api/update-budget
...
d = Diagram(T,
            title="PUT /api/update-budget — detailed implementation workflow",
            subtitle="Level 2 · real functions, middleware and models · badges map to the "
                     "11 stages in budget-api-03-update-budget-overview.svg")

r1 = d.region(20,   272, "User Interface", "Edit modal in the insights header",
              accent="ui", step=1)
...
a2 = d.card(*col(r1, 1), "ui", "layout", "COMPONENT", "Insights Header",
            "Header.js",
            "Pencil button opens an inline edit modal over the summary.", step="01")
a3 = d.card(*col(r1, 2), "ui", "cursor", "HANDLER", "Submit Handler",
            "handleBudgetSubmit(e)",
            "Form submit; the input is required with min 0 in the markup.", step="01")
...
b2 = d.card(*col(r2, 1), "frontend", "send", "MUTATION FN", "Mutation Function",
            "updateBudget(budget)",
            "Sends newBudget straight from state — a string, not a number.", step="02")
b3 = d.card(*col(r2, 2), "frontend", "send", "API CLIENT", "Request Function",
            "api.put('/api/update-budget')",
...
c2 = d.card(*col(r3, 1), "backend", "server", "ROUTER", "Route Match",
            "api.routes.js",
            "PUT /update-budget → verifyToken → updatebudget. No validator.", step="04")
...
h1 = d.card(1420, LY + 0 * PITCH, "ui", "monitor", "MODAL", "Modal Closes",
            "setEditBudget(false)",
            "No success toast on this path — only an error one exists.", step="11")
h2 = d.card(1420, LY + 1 * PITCH, "ui", "sigma", "UI", "Header Figure",
            "₹ {totalBudget}",
...
svg = d.render(
    ...
    meta_left="docs/api-workflows · BUDGET-03 · Level 2 detailed",
    footer_notes=[
